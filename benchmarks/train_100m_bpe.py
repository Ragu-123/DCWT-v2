"""
HALE — 100M Parameter Scale Test on OpenWebText
===============================================
Scale both Standard Transformer and accelerated architecture to ~100M params
...
Previous results (6-8M params, BPE 8K vocab, WikiText-2):
  ...
  Accelerated model:    target improved gap over historical Wave baseline
...
def train_model(model, train_data, val_data, tok, vocab_size, device,
                ...
                peak_lr=0.0001, use_amp=True, autocast_device="cuda",
                autocast_dtype=torch.float16, save_dir="checkpoints",
                bio_cfg: Optional[Dict[str, Any]] = None):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, (x, y) in enumerate(train_loader, start=1):
            ...
            if step % 500 == 0:
                print(f"    Step {step}/{len(train_loader)} | Loss {tl/nb:.4f}", flush=True)
        ...
        if epoch % 2 == 0 or epoch == num_epochs:
            ...
            for seed in seeds:
                text = generate_text(model, tok, seed, device, max_tokens=40, bio_cfg=bio_cfg)
                print(f"    [{seed}] -> {text}")
...
def main():
    ...
    model2_save_dir = "100m_dcwt_v2_checkpoints" if use_dcwt else "100m_hale_checkpoints"
    model2_results_path = "100m_dcwt_v2_results.json" if use_dcwt else "100m_hale_results.json"
    model2_complexity = "O(n log n)" if use_dcwt else "O(n·w + n·d^2)"

    print("=" * 70)
    print("  100M PARAMS / 32K VOCAB / OPENWEBTEXT")
    print(f"  Scaling test for {model2_name}")
    if use_dcwt:
    ...
    for seed in seeds:
        text = generate_text(std_model, tok, seed, device, max_tokens=50, bio_cfg={"enabled": False})
        print(f"  [{seed}] -> {text}")
    ...
    # ============================================================
    # MODEL 2: Accelerated architecture 100M (HALE default)
    # ============================================================
    print(f"\n{'='*70}")
    print(f"  MODEL 2: {model2_name} ~100M")
    print(f"{'='*70}")
    ...
    wave_result = train_model(
        ...
        f"{model2_name} 100M", num_epochs=num_epochs,
        batch_size=batch_size, grad_accum=grad_accum,
        peak_lr=peak_lr, use_amp=use_amp,
        autocast_device=autocast_device, autocast_dtype=autocast_dtype,
    ...
    print(f"\n  {'Metric':<25} {'Std 100M':>18} {model2_name + ' 100M':>18} {'Winner':>10}")
    print(f"  {'-'*25} {'-'*18} {'-'*18} {'-'*10}")
    ...
    print(f"  {'Complexity':<25} {'O(n^2)':>18} {model2_complexity:>18}")
    ...
    if w_tp < s_tp:
        ...
        print(f"\n  {model2_name} BEATS Standard Transformer by {pct:.1f}%!")
    else:
